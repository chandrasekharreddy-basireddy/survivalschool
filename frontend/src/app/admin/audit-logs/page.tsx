  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      <div className="card mt-6 overflow-x-auto p-0">
          ...
          <tbody>
            {(logs || []).map((l) => (
              <tr key={l.id} className="border-b border-ink-800 last:border-0">
                <td className="px-4 py-3 text-fg-muted">{formatDateTime(l.created_at)}</td>
                <td className="px-4 py-3 text-fg">{l.action}</td>
        ...
        {logs !== null && logs.length === 0 && <p className="p-6 text-center text-sm text-fg-subtle">No matching audit log entries.</p>}
      </div>
    </div>
  );
}
