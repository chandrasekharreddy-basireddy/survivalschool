  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-md px-6 py-24 text-center">
        <p className="text-fg-muted">Sign in to see your certificates.</p>
          ...
          {certs.map((c) => (
            <div key={c.certificate_number} className={`card ${c.revoked ? "opacity-50" : ""}`}>
              <div className="flex items-start justify-between gap-2">
                <h2 className="font-semibold text-fg">{c.course_title}</h2>
              ...
              {c.skills.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {c.skills.slice(0, 4).map((s) => (
                    <span key={s} className="rounded bg-ink-800 px-2 py-0.5 text-[11px] text-fg-muted">{s}</span>
                  ))}
                </div>
              )}
